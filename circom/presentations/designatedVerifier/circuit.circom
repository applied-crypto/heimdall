include "../../lib/metaDataDesignatedVerifier.circom"
include "../../lib/contentData.circom"

template DesignatedVerifier(depth, revocationDepth) {
	/*
	* Private Inputs
	*/
	// Meta
	...
	signal private input possessionVerifierSK;
	signal private input signatureDesignatedVerifier[3];
	// Content
	...
	*/
	...
	signal input verifierPK[2]; //10 11
	/*
	* Meta Calculations
	*/
	//Designated Verifier
	possessionVerifierSK * (possessionVerifierSK - 1) == 0;
    component eddsaDVVerify = EdDSAPoseidonVerifier();
    eddsaDVVerify.enabled <== possessionVerifierSK; 
    eddsaDVVerify.Ax <== verifierPK[0];
    eddsaDVVerify.Ay <== verifierPK[1];
    eddsaDVVerify.R8x <== signatureDesignatedVerifier[0];
    eddsaDVVerify.R8y <== signatureDesignatedVerifier[1];
    eddsaDVVerify.S <== signatureDesignatedVerifier[2];
    eddsaDVVerify.M <== challenge;
	...
	checkMetaDataIntegrity.checkSignature <== 1 - possessionVerifierSK;
...
component main = DesignatedVerifier(4, 13);
